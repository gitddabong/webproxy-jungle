...
int main(int argc, char **argv){
  ...
  while (1) {
    // wawit for connection as a server
     ...
     * remove host part http://localhost:80002
    ...
    sprintf(s_buf, "%s %s %s\n", method, path, version);
    Rio_writen(cliendfd, s_buf, strlen(s_buf));
    printf("%s", s_buf);
     ...
     */
    while ((cn = Rio_readlineb(&cliend_rio, c_buf, MAXLINE)) != 0)
 ...
 * if not, return false
 */
int block_uri(char *uri, char list[MAXENTRY][MAXLINE]) {
  int i;
  for (i = 0; list[i][0] != '\0'; i++)
    if (strcmp(uri, list[i], strlen(uri)) == 0)
      return 1;
  return 0;
}